...
func (u *usecase) removeGenerator(inputEntity generator.OnlineGeneratorInputEntity) error {
	...
	genDomainRepositoryFilePath := path.Join(inputEntity.ProjectName, "internal", "domain", "gen_repositoty_interface.go")
	...
	if err := os.RemoveAll(genDomainUsecaseFilePath); err != nil {
		return err
	}
	if err := os.RemoveAll(genDomainRepositoryFilePath); err != nil {
		return err
	}
	return nil
}
